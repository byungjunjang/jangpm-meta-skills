...
- Codex 배포: `.codex/skills/` 또는 사용자 전역 `~/.codex/skills/`
...
네 개의 스킬 폴더를 `~/.codex/skills/`에 복사합니다.
...
- 이 저장소는 커스텀 서브에이전트를 아직 포함하지 않으므로 `.codex/agents/` 디렉터리는 기본적으로 체크인하지 않습니다. 필요할 때만 `.codex/agents/`를 추가하면 됩니다.
